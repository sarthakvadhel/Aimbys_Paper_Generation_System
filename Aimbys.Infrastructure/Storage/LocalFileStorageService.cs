...
public class LocalFileStorageService : ILocalFileStorageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new FileStorageException("No bytes were provided.");
        }
        ...
        {
            throw new FileStorageException("actorUserId is required.");
        }
        ...
        var ext = SanitiseExtension(fileName);
        var storedFileName = $"{guidPart}{hashPrefix}{ext}";
        var finalPath = Path.Combine(areaFolder, storedFileName);
        ...
        var asset = new FileAsset
        {
            ...
            OwnerKey = ownerKey,
            InstituteId = instituteId,
            OriginalFileName = SanitiseDisplayName(fileName),
            StoredFileName = storedFileName,
        ...
        _db.AuditLogs.Add(new AuditLog
        {
            ...
            EntityType = nameof(FileAsset),
            EntityId = asset.Id.ToString(),
            DetailsJson = $"{{\"area\":\"{area}\",\"sizeBytes\":{asset.SizeBytes},\"sha256\":\"{sha256Hex}\",\"source\":\"server\"}}"
        });
        await _db.SaveChangesAsync(cancellationToken);

        _logger.LogInformation(
            "Server-generated file saved. Token={Token} Area={Area} OwnerKey={OwnerKey} Bytes={Bytes} Sha256={Sha256}",
            asset.Token, area, ownerKey, asset.SizeBytes, sha256Hex);
